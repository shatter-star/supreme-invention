...
async def style_transfer(content_image: UploadFile = File(...), style_images: List[UploadFile] = File(...)):
    # Creating the 'data/tmp' directory if it doesn't exist
    tmp_dir = os.path.join("data", "tmp")
    os.makedirs(tmp_dir, exist_ok=True)

    content_image_path = os.path.join(tmp_dir, content_image.filename)
    style_images_paths = [os.path.join(tmp_dir, style_image.filename) for style_image in style_images]
